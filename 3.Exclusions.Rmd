...
dat %>%
  ...
  pull(ID) %>%
  write.table(paste0(filepath_misc, "NBR_IDs_no_cohort_info.csv"))
```
...
```
356 participants did not report their biological sex
...
```
A further 422 participants did not report their age
...
```
An additional 2182 individuals excluded because they are missing an item on the mdq
...
```
725 participants did not report their biological sex
...
```
A further 2079 participants did not report their age
...
```
An additional 6464 individuals excluded because they are missing an item on the MDQ.
