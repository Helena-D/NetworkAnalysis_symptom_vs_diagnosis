...
dup_people <- read_csv(file = paste0(filepath_own, "/duplicate_participant_report_2021_11_30_hashed.csv"))
# 882 people
```

Filter out people in main dataset with same ID
```{r}
dat <- readRDS(file = paste0("../data/dat_merged_2021-11-30.rds"))
dim(dat)
colnames(dat)
...
EDGI_dat_ED100k <- same_people_EDGI_dat %>%
  ...
  pull(ID) # 525 people (out of a total 882 EDGI IDs that I have in my data. Therefore, there are 357 people who have EDGI IDs but not EDGI ED100K data)
