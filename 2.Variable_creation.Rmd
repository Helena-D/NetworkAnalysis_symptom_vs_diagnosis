...
dat <- readRDS(file = paste0("../../data/dat_merged_2023-04-11.rds")) 
dim(dat) 
colnames(dat)
```
...
IDs_to_drop <- readRDS(file = paste0("../../data/GLAD_EDGI_IDs_to_drop_dat_2023-04-05.rds")) 
dim(IDs_to_drop)
...
dat <- dat %>%
  mutate(anorexia_nervosa_with_binge =
           case_when(ed.DSM5_AN_with_binge_binary_numeric == 1 |
                      ed.DSM5_AN_with_binge_binary_numeric_cop == 1 ~ 1
                     )
...
# Check
dat %>%
  freq(anorexia_nervosa_with_binge)
...
# Check
dat %>%
  freq(binge_eating_disorder_algorithm_derived)
```
...
dat <- dat %>%
  mutate(ED_hierarchical_sens_ANB =
           case_when(anorexia_nervosa_with_binge == 1 ~ "Anorexia with binge eating",
                     bulimia_nervosa == 1 ~ "Bulimia nervosa",
                     binge_eating_disorder == 1 ~ "Binge-eating disorder"
                     )
         )
...
dat <- dat %>%
  mutate(ED_hierarchical_sens_ANB_numeric =
           case_when(ED_hierarchical_sens_ANB == "Anorexia with binge eating" ~ 2,
                     ED_hierarchical_sens_ANB == "Bulimia nervosa" ~ 1,
                     ED_hierarchical_sens_ANB == "Binge-eating disorder" ~ 0
                     )
         )
...
```{r select variables that change}
dat_diagnostic_crossover <- dat %>%
 filter(
        (
          (ed.DSM5_BED_binary_numeric == 1 |
          an.1.bingeeating_disorder_numeric == 1 |
          mhd.bed_numeric == 1) & # Has BED in GLAD or EDGI UK (sign-up)
          ...
          is.na(ed.DSM5_BN_binary_numeric)) &
            (an.1.bulimia_nervosa_numeric == 0 |
          is.na(an.1.bulimia_nervosa_numeric)) &
            (mhd.bn_numeric == 0 |
          is.na(mhd.bn_numeric)))) & # Does not have BN or ANBP in GLAD or EDGI UK (sign-up)
   
         (ed.DSM5_AN_binge_purge_binary_numeric_cop == 1 |
           ed.DSM5_BN_binary_numeric_cop == 1 |
           mhd.bn_cop_numeric == 1)) # Has BN or ANBP in COPING
    

dat_diagnostic_crossover2 <- dat %>%
 filter(
        (((ed.DSM5_BED_binary_numeric == 1 |
          an.1.bingeeating_disorder_numeric == 1 |
          mhd.bed_numeric == 1) | 
          ...
          an.1.bulimia_nervosa_numeric == 1 |
           mhd.bn_numeric == 1)) & # Has BED or BN in GLAD or EDGI UK (sign-up)
           
         ((ed.DSM5_AN_binge_purge_binary_numeric == 0 |
          is.na(ed.DSM5_AN_binge_purge_binary_numeric)))) & # Does not have ANBP in GLAD or EDGI UK (sign-up)
   
         ed.DSM5_AN_binge_purge_binary_numeric_cop == 1 
        ) # Has ANBP in COPING
    
dat_diag_cross_check <- dat %>%
  filter(ID %in% dat_diagnostic_crossover$ID |
         ID %in% dat_diagnostic_crossover2$ID)

dat_diag_cross_check <- dat_diag_cross_check %>%
  mutate(BED_in_GLAD_EDGI =
           case_when(
              ed.DSM5_BED_binary_numeric == 1 |
          an.1.bingeeating_disorder_numeric == 1 |
          mhd.bed_numeric == 1~ 1))
...
dat_diag_cross_check <- dat_diag_cross_check %>%
  mutate(BED_in_COPING =
           case_when(
              ed.DSM5_BED_binary_numeric_cop == 1 |
          mhd.bed_cop_numeric == 1~ 1))

dat_diag_cross_check <- dat_diag_cross_check %>%
  mutate(BN_in_GLAD_EDGI =
           case_when(
              ed.DSM5_BN_binary_numeric == 1 |
                an.1.bulimia_nervosa_numeric == 1 |
                mhd.bn_numeric == 1 ~ 1))
...
dat_diag_cross_check <- dat_diag_cross_check %>%
  mutate(BN_in_COPING =
           case_when(
             ed.DSM5_BN_binary_numeric_cop == 1 |
           mhd.bn_cop_numeric == 1  ~ 1))

dat_diag_cross_check <- dat_diag_cross_check %>%
  mutate(ANBP_in_GLAD_EDGI =
           case_when(
              ed.DSM5_AN_binge_purge_binary_numeric == 1  ~ 1))

dat_diag_cross_check <- dat_diag_cross_check %>%
  mutate(ANBP_in_COPING =
           case_when(
              ed.DSM5_AN_binge_purge_binary_numeric_cop == 1 ~ 1))


dat_diag_cross_check %>%
  select(BED_in_GLAD_EDGI,
         BED_in_COPING,
         BN_in_GLAD_EDGI,
         BN_in_COPING,
         ANBP_in_GLAD_EDGI,
...
# Check number of people in final file
symptom_dat <- readRDS(file = "../../data/symptom_dat_analysis2022-03-15.rds")

dat_diag_cross_check <- dat_diag_cross_check %>%
  filter(ID %in% symptom_dat$ID)

# How many of these people are in the ANBP group?
dat_diag_cross_check_ANBP <- dat_diag_cross_check %>%
  filter(ed.DSM5_AN_binge_purge_binary_numeric_cop == 1 |
           ed.DSM5_AN_binge_purge_binary_numeric == 1 ) # 82 people with ANBP

# How many of these people are in the BN group?
dat_diag_cross_check %>%
  filter(ID %!in% dat_diag_cross_check_ANBP$ID &
          (ed.DSM5_BN_binary_numeric == 1 |
           an.1.bulimia_nervosa_numeric == 1 |
           mhd.bn_numeric == 1 |
           mhd.bn_cop_numeric == 1 |
           ed.DSM5_BN_binary_numeric_cop == 1)) # 284 people with BN

# Check their values
dat_diag_cross_check_table <- dat_diag_cross_check %>%
  select(dem.bmi_signup_cop,
         dem.bmi_signup,
         dem.dob_age,
         dem.dob_age_cop,
...
# Hierarchically for lowest BMI
dat <- dat %>%
  mutate(binge_eating_disorder_timing_hierarchy = 
           case_when(
             
             # GLAD or EDGI sign-up
             # ANBP reported (GLAD, EDGI)
             ed.DSM5_AN_binge_purge_binary_numeric == 1 ~ "GLAD/EDGI",
             
            # ANBP reported (NBR)
             ed.DSM5_AN_binge_purge_binary_numeric_cop == 1  ~ "NBR",
            
            # BN reported (GLAD, EDGI) 
            ed.DSM5_BN_binary_numeric== 1 |
             an.1.bulimia_nervosa_numeric == 1 |
              mhd.bn_numeric ==1 ~ "GLAD/EDGI",
            
             # BN reported (NBR) 
            ed.DSM5_BN_binary_numeric_cop == 1 |
            mhd.bn_cop_numeric== 1 ~ "NBR",
             
            # BED reported (GLAD, EDGI) 
           an.1.bingeeating_disorder_numeric == 1 |
           mhd.bed_numeric == 1 |
           ed.DSM5_BED_binary_numeric == 1 ~ "GLAD/EDGI",
   
           # BED reported (NBR) 
            mhd.bed_cop_numeric == 1 |
          ed.DSM5_BED_binary_numeric_cop == 1 ~ "NBR"
          ))
          
            
# Check 
dat %>%
  freq(binge_eating_disorder_timing_hierarchy)
...
# Lowest BMI (would rather lowest BMI at ANY height [dem.bmi_an_lowest/dem.bmi_an_lowest_cop], than their lowest BMI at adult height if poss [dem.bmi_lowest/dem.bmi_lowest_cop]). Here, I have used specific timing of disorder because the differences between the disorders is lowest BMI.
dat <- dat %>%
  mutate(bmi_lowest_dx_level = 
           case_when(
             binge_eating_disorder_timing_hierarchy == "GLAD/EDGI" &  
             ...
             binge_eating_disorder_timing_hierarchy == "NBR" & 
...
dat %>%
  filter(is.na(bmi_lowest_dx_level) &
           !is.na(binge_eating_disorder_timing_hierarchy)) %>%
